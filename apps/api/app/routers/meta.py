...
def modalities() -> list[dict]:
    return [{"value": m.value, "label": MODALITY_LABELS.get(m.value, m.value)} for m in ModalityCode]


@router.get("/statuses")
def statuses() -> list[dict]:
    return [{"value": s.value, "label": s.value.replace("_", " ").title()} for s in OpportunityStatus]
